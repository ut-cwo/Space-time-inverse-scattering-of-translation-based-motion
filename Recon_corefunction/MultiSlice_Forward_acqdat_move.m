...
function [efield1, efield2,efield_vol] = MultiSlice_Forward_acqdat_move(obj, psz, xx, yy, dfx, prop_phs, NA_crop, lambda, fx_in, fy_in, z_plane, pdar, use_gpu, xmove, ymove)

    obj = subpixelshift(obj, xmove, ymove);
    % initalizing outpts
    ...
    prop_kernel = conj(exp( (size(obj, 3)/2) .* prop_phs .* psz) );
...
prop_kernel = exp( ((size(obj, 3)/2).* psz+z_plane) .* prop_phs);
fU_current = fft2(U_in);
U_in2  = ifft2(fU_current.* prop_kernel);


    U_in2 = U_in2((pdar+1):(end-pdar),(pdar+1):(end-pdar),:);
    efield1 = efield.*conj(U_in2);
    efield2 = efield;
end
